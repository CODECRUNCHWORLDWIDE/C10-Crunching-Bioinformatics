...
```mermaid
flowchart TD
  A["Scan BAMs for active regions"] --> B["Assemble local haplotypes with a De Bruijn graph"]
  B --> C["Realign reads to candidate haplotypes with a pair HMM"]
  C --> D["Compute TLOD and NLOD per candidate variant"]
  D --> E["Emit unfiltered VCF"]
```
*Mutect2's internal active-region haplotype-assembly flow, from raw pileup to unfiltered VCF.*
...
```mermaid
flowchart LR
  A["configureStrelkaSomaticWorkflow.py"] --> B["Creates run directory and runtime config"]
  B --> C["runWorkflow.py performs the calling"]
  C --> D["somatic snvs vcf gz"]
  C --> E["somatic indels vcf gz"]
```
*Strelka2's two-phase workflow: configure then run, producing separate SNV and indel VCFs.*
