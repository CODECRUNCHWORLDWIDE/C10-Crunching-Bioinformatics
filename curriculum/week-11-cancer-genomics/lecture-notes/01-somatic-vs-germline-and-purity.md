...
```mermaid
flowchart TD
  N["Normal cell"] --> F["Founding clone gains a driver mutation"]
  F --> T["Truncal mutations present in every tumor cell"]
  T --> SA["Subclone A gains an additional driver"]
  T --> SB["Subclone B gains an additional driver"]
  SA --> PA["Passenger mutations in subclone A"]
  SB --> PB["Passenger mutations in subclone B"]
```
*Clonal evolution: truncal mutations founder every tumor cell, while subclones branch off and accumulate their own drivers and passengers.*
...
```mermaid
flowchart TD
  Var["Variant observed in tumor reads"] --> Chk{"Present in matched normal at similar allele frequency"}
  Chk -->|Yes| Germ["Classify as germline and remove from candidates"]
  Chk -->|No| Som["Classify as somatic candidate"]
  Som --> Chk2["Check panel of normals and germline resource"]
  Chk2 --> Out["FilterMutectCalls assigns PASS or a filter reason"]
```
*How the matched-normal comparison sorts a candidate variant into germline versus somatic before the filter set runs.*
