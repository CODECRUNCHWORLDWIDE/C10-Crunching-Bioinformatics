...
sequenceDiagram
  ...
  Client->>Cache: check variant id
  Cache-->>Client: cache miss
  Client->>gnomAD: POST query
  gnomAD-->>Client: 429 too many requests
  Client->>Client: sleep and backoff
  Client->>gnomAD: retry POST query
  gnomAD-->>Client: 200 with variant data
  Client->>Cache: store response
```
*Client checks the cache first, then retries gnomAD with exponential backoff before caching the result.*
...
```mermaid
flowchart LR
  A["VEP annotated VCF CSQ field"] --> D["annotate variants function"]
  B["ClinVar local VCF"] --> D
  C["gnomAD API or cache"] --> D
  D --> E["Per variant pandas DataFrame"]
```
*Lecture 2 joins VEP consequence, ClinVar assertions, and gnomAD frequency into one row per variant.*
