...
```mermaid
flowchart TD
  E["Evidence criteria collected"] --> BA{"Any BA1 hit"}
  BA -->|Yes| Benign["Benign"]
  BA -->|No| PVS{"PVS1 plus PM2 or two PP"}
  PVS -->|Yes| Path["Pathogenic or Likely Pathogenic"]
  PVS -->|No| PSonly{"Two or more PS hits"}
  PSonly -->|Yes| Path
  PSonly -->|No| BSonly{"Two or more BS hits"}
  BSonly -->|Yes| Benign
  BSonly -->|No| VUS["Uncertain Significance"]
```
*Simplified view of how BA1, PVS1, PS, and BS evidence counts resolve to an ACMG tier.*
...
```mermaid
flowchart LR
  A["VCF restricted to CPIC genes"] --> B["Call star alleles Aldy or Stargazer"]
  B --> C["Map diplotype to metabolizer phenotype"]
  C --> D["Look up CPIC recommendation per drug"]
  D --> E["Per drug report"]
```
*The five-step CPIC pharmacogenomics workflow, from raw VCF to a per-drug dosing recommendation.*
