...
```mermaid
flowchart LR
  A["FASTQ"] --> B["Trim fastp"]
  B --> C["Align bwa mem"]
  C --> D["Sort and markdup"]
  D --> E["Call variants"]
  E --> F["Filter to PASS VCF"]
  F --> G["Annotate VEP and SnpEff"]
  G --> H["Cross join ClinVar gnomAD dbSNP"]
  H --> I["Report"]
```
*Week 8 picks up where Week 6 left off: annotate, cross-join, report.*
...
```mermaid
flowchart TD
  V["Variant"] --> A1["Functional consequence VEP or SnpEff"]
  V --> A2["Population frequency gnomAD"]
  V --> A3["Clinical knowledge ClinVar and dbSNP"]
  A1 --> R["Per variant report row"]
  A2 --> R
  A3 --> R
```
*Three independent axes feed one report row; none alone is a clinical interpretation.*
