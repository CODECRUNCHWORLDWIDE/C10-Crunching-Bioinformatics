...
- **1000 Genomes Project, phase 3** — 2,504 individuals from 26 populations. Open access. Consent explicitly covers research use including by learners.
...
```mermaid
flowchart TD
  A["Public bioinformatics archives"] --> B["NCBI United States"]
  A --> C["EBI and Ensembl United Kingdom"]
  A --> D["Project specific archives"]
  B --> B1["GenBank RefSeq SRA dbSNP ClinVar PubMed"]
  C --> C1["ENA UniProt Ensembl REST API VEP"]
  D --> D1["1000 Genomes GTEx TCGA GISAID UCSC"]
```
*Where each dataset in this lecture lives: two big general archives plus several project-specific ones.*
...
A friend, a biology Ph.D. learner, casually offers you a small VCF file from their lab's project. Two samples. They say "it's de-identified, just play with it for practice." What do you do?

The honest answer:

1. **Ask what consent the donors gave.** If "research use including by learners at our institution," the lab's IRB should be able to confirm. If unsure, the answer is no.
...
```mermaid
flowchart TD
  A["Friend offers a VCF file"] --> B{"Consent confirmed by IRB"}
  B -->|"No or unsure"| F["Decline"]
  B -->|"Yes"| C{"Same data already public"}
  C -->|"No"| F
  C -->|"Yes"| D["Use the public version with its DOI"]
```
*The decision path for the friend-with-a-VCF scenario: any unresolved question routes straight to decline.*
