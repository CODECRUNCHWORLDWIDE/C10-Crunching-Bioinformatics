...
```mermaid
flowchart LR
  A["FASTA draft contigs"] --> B["Medaka polish ONT or DeepConsensus HiFi"]
  B --> C["FASTA polished"]
  C --> D["asmstats N50 and L50"]
  C --> E["BUSCO gene completeness"]
  C --> F["Bandage graph topology"]
  C --> G["QV vs reference"]
  D --> H["QC report"]
  E --> H
  F --> H
  G --> H
```
*Polishing feeds four parallel QC checks that converge on one report.*
...
The Week 10 exercises and mini-project do not invoke DeepConsensus (it requires GPU access most learners do not have); the Hifiasm challenge runs on `badread`-simulated HiFi-style reads whose accuracy already matches DeepConsensus output. The call is documented here for production reference.
