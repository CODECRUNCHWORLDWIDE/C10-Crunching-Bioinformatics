...
```mermaid
flowchart LR
  A["Long reads"] --> B["Overlap find aligned read pairs"]
  B --> C["Layout clean the overlap graph"]
  C --> D["Consensus call bases per path"]
  D --> E["Contigs"]
```
*The three OLC stages that every long-read assembler runs, in order.*
...
```mermaid
flowchart TD
  A["Input platform"] --> B["ONT R10.4.1 Dorado SUP"]
  A --> C["ONT R9.4.1 legacy"]
  A --> D["PacBio HiFi"]
  A --> E["PacBio CLR legacy"]
  A --> F["Mixed ONT and HiFi"]
  B --> B2["Flye nano-hq"]
  C --> C2["Flye nano-raw"]
  D --> D2["Hifiasm"]
  E --> E2["Canu pacbio legacy"]
  F --> F2["Verkko"]
```
*Choosing an assembler based on the input platform and chemistry.*
