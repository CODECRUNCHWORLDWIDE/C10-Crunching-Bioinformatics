...
```mermaid
flowchart LR
  A["Read k-mers"] --> B["Hash lookup per k-mer"]
  B --> C["Color set per k-mer"]
  C --> D["Intersect all color sets"]
  D --> E["Compatibility class"]
  E --> F["EM estimates abundance"]
```
*Pseudoalignment builds a compatibility class from k-mer lookups alone, with no base-by-base alignment.*
...
```mermaid
flowchart TD
  A["Read maps to multiple transcripts"] --> B{"Which pipeline"}
  B -->|"featureCounts default"| C["Discard the read"]
  B -->|"featureCounts with -M --fraction"| D["Split fractionally by transcript count"]
  B -->|"kallisto or Salmon EM"| E["Split fractionally by estimated abundance"]
```
*Three ways a multi-mapping read gets resolved into per-transcript or per-gene counts.*
