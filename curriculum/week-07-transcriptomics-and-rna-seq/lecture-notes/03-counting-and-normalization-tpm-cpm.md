...
```mermaid
flowchart LR
  A["Per-transcript counts from kallisto or Salmon"] --> B["Merge with transcript to gene map"]
  B --> C["Group by gene id and sum"]
  C --> D["Per-gene counts matrix"]
  D --> E["Raw counts to DESeq2 or edgeR"]
  D --> F["log2 TPM to heatmap or PCA"]
```
*Per-transcript quantification is aggregated to a per-gene matrix, then split into a raw-count path for differential expression and a normalized path for visualization.*
