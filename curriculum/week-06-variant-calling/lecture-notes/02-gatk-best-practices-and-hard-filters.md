...
```mermaid
flowchart TD
  A["Active region detection"] --> B["Local assembly with De Bruijn graph"]
  B --> C["Likelihood computation with pair HMM"]
  C --> D["Genotype assignment"]
  D --> E["Variant calls from chosen haplotypes"]
```
*The four-stage local-reassembly algorithm inside GATK HaplotypeCaller.*
...
```mermaid
flowchart LR
  A["Sorted markdup BAM"] --> B["Variant calling"]
  B --> C["Hard filtering"]
  C --> D["Normalization"]
  D --> E["VEP annotation"]
  E --> F["PASS only extraction"]
  F --> G["bcftools stats summary"]
```
*The six-stage Week 6 pipeline from BAM to an annotated, QC-summarized VCF.*
