...
```mermaid
flowchart LR
  A["BLOCKS database of conserved regions"] --> B["Cluster by percent identity"]
  B --> C["Count substitution frequencies"]
  C --> D["Compute log odds ratio"]
  D --> E["Scale and round to an integer score"]
```
*How raw BLOCKS alignments turn into a single BLOSUM substitution score.*
...
```mermaid
flowchart TD
  A["Is the sequence DNA or protein"] -->|"DNA"| B["Use plus one minus one or a transition aware scheme"]
  A -->|"Protein"| C{"Do you know the divergence"}
  C -->|"Closely related"| D["Use BLOSUM80 or PAM30"]
  C -->|"Distantly related"| E["Use BLOSUM45 or PAM250"]
  C -->|"Unknown"| F["Use BLOSUM62 as the safe default"]
```
*The practical decision tree for picking a substitution matrix.*
