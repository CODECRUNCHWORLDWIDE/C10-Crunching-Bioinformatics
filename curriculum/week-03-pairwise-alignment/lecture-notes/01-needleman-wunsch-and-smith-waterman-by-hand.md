...
```mermaid
flowchart TD
  A["Cell H at row i col j"] --> B{"Pick the best predecessor"}
  B -->|"Diagonal"| C["Align A i with B j - match or mismatch"]
  B -->|"Up"| D["Gap in B - residue vs dash"]
  B -->|"Left"| E["Gap in A - dash vs residue"]
  C --> F["H i j is the max of the three"]
  D --> F
  E --> F
```
*How each cell's score is chosen from its three possible predecessors.*
...
```mermaid
flowchart TD
  A["Compute diagonal up and left as in Needleman-Wunsch"] --> B{"Is the best of the three negative"}
  B -->|"Yes"| C["Reset the cell to zero - start a fresh local alignment"]
  B -->|"No"| D["Keep the computed score"]
  C --> E["Traceback starts at the global max and stops at a zero"]
  D --> E
```
*The zero floor: a cell that would go negative restarts local alignment instead of carrying the loss forward.*
