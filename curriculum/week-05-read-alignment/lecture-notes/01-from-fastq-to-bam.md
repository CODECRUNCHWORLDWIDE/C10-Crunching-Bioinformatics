...
```mermaid
flowchart TD
  A["Seed - find MEMs via FM-index"] --> B["Reseed - split long MEMs"]
  B --> C["Chain - group colinear MEMs"]
  C --> D["Extend - banded Smith-Waterman"]
  D --> E["Score - pick primary alignment"]
```
*The five stages of BWA-MEM's seed-and-extend algorithm, each stage filtering down what the next stage has to process.*
...
```mermaid
flowchart LR
  A["FASTQ reads"] --> B["bwa index reference"]
  B --> C["bwa mem align"]
  C --> D["samtools sort"]
  D --> E["sorted BAM"]
  E --> F["samtools index"]
  F --> G["flagstat and depth QC"]
  G --> H["samtools markdup"]
```
*Data flow from raw reads through alignment, sorting, indexing, QC, and duplicate marking.*
