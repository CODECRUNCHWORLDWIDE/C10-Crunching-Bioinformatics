...
```mermaid
flowchart TD
  A["Hundreds of millions of database sequences"] --> B["Seed: short word matches"]
  B --> C["Extend: X-drop ungapped extension"]
  C --> D["Score: gapped Smith-Waterman extension"]
  D --> E["Report: E-value ranked hits"]
```
*Each stage of the seed-and-extend pipeline filters out most of the candidates before the next, more expensive stage runs.*
...
Note the asymmetry. **A small E-value is evidence against the null** (the hit is unlikely under chance). A small E-value is *not* "the probability the hit is wrong" — that is a Bayesian claim that requires a prior, which the E-value does not give you. This nuance routinely trips up biology learners learning BLAST for the first time. Re-read the previous paragraph until it sticks.
...
```mermaid
flowchart TD
  A["Raw alignment score S"] --> B["Bit score: normalized by K and lambda"]
  A --> C["E-value: combines score with database size"]
  B --> D["Comparable across matrices and databases"]
  C --> E["Expected number of chance hits in this database"]
```
*The same raw score feeds two different normalizations that answer two different questions.*
