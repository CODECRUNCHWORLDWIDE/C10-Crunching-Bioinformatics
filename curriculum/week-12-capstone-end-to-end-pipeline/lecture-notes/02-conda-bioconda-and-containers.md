...
```mermaid
flowchart TD
  A["Workflow manager: Snakemake or Nextflow"] --> B["Package manager: Conda plus bioconda"]
  B --> C["Container runtime: Singularity or Apptainer"]
  C --> D["OS: Linux kernel and glibc"]
```
*Each layer pins the layer beneath it, from the workflow DAG down to the OS kernel.*
