...
```mermaid
flowchart TD
  A["Pick a capstone track"] --> B["Track 1 Variant discovery: measure caller accuracy against GIAB"]
  A --> C["Track 2 RNA-seq DE: most publishable portfolio plots"]
  A --> D["Track 3 MSA and phylogeny: shortest runtime, gentlest on hardware"]
  A --> E["Track 4 Long-read assembly: heaviest RAM, most algorithmic"]
```
*Track choice trades off hardware, runtime, and portfolio value; pick one and commit.*
...
```mermaid
flowchart TD
  A["Push final commit to GitHub"] --> B["Spin up a clean Linux VM"]
  B --> C["Install Apptainer"]
  C --> D["Clone the repo"]
  D --> E["Build the container"]
  E --> F["Run the pipeline"]
  F --> G["Compare output hashes"]
  G --> H["Match: capstone passes"]
  G --> I["Mismatch: find the non-determinism"]
```
*The seven-step clean-machine test that decides whether the capstone is byte-identically reproducible.*
