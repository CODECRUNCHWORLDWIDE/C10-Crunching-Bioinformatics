...
```mermaid
flowchart TD
  A["Start with star tree one central node N leaves"] --> B["Compute Q value for every pair"]
  B --> C["Pick pair with smallest Q and merge into new node"]
  C --> D["Compute new node distance to remaining taxa"]
  D --> E{"More than two nodes remain"}
  E -->|Yes| B
  E -->|No| F["Join final two nodes with single branch"]
```
*Neighbor-joining iteratively merges the closest pair until one unrooted tree remains.*
...
```mermaid
flowchart TD
  A["Resample alignment columns with replacement"] --> B["Rebuild tree from resampled alignment"]
  B --> C["Repeat for B replicates"]
  C --> D["Count fraction of replicate trees with same branch"]
  D --> E["Bootstrap percentage per internal branch"]
```
*The bootstrap protocol resamples columns to test how robust each branch is.*
