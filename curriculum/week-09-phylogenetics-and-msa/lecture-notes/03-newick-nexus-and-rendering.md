...
```mermaid
flowchart LR
  A["Tree in memory Biopython Tree object"] --> B["Newick file"]
  B --> C["Optional Nexus file"]
  C --> D["Rendered figure ASCII PNG or SVG"]
```
*Lecture 3 covers turning an in-memory tree into files and figures.*
...
```mermaid
flowchart TD
  Root --> Clade["Clade branch length 0.10"]
  Root --> Mouse["mouse branch length 0.20"]
  Clade --> Human["human branch length 0.05"]
  Clade --> Chimp["chimp branch length 0.05"]
```
*The nested parentheses in the Newick string map directly onto this tree shape.*
