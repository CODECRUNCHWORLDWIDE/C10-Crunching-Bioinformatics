...
```mermaid
flowchart LR
  A["FASTA input"] --> B["MSA MAFFT Clustal Omega or MUSCLE 5"]
  B --> C["Aligned FASTA gaps inserted"]
  C --> D["Column trim drop gappy columns"]
  D --> E["Distance matrix Jukes Cantor or K2P"]
  E --> F["Tree NJ or ML"]
  F --> G["Newick or Nexus file"]
  G --> H["Rendered PNG or SVG"]
```
*The Week 9 pipeline from raw FASTA to a rendered tree; Lecture 1 covers the first three boxes.*
...
```mermaid
flowchart TD
  A["Pairwise distances between all sequences"] --> B["Build a guide tree"]
  B --> C["Align the two closest sequences first"]
  C --> D["Add the next closest sequence or profile"]
  D --> E["Repeat leaves to root along the guide tree"]
  E --> F["Final progressive alignment"]
```
*Progressive alignment builds outward from the closest pairs toward the full alignment.*
